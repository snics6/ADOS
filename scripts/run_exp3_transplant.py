#!/usr/bin/env python
"""Experiment 3: transplant other-task rank-1 onto this task's video.

    ./venv/bin/python -u scripts/run_exp3_transplant.py --labels-only
    ./venv/bin/python -u scripts/run_exp3_transplant.py
    ./venv/bin/python -u scripts/run_exp3_transplant.py --smoke
"""
...
from ados_ffm.exp3_transplant import (  # noqa: E402
    expand_sides,
    load_exp2_fdr,
    lock_labels,
    run_side,
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "outputs/exp3")
    ...
    if args.smoke:
        n_perm = min(n_perm, 5)
        if args.out == ROOT / "outputs/exp3":
            args.out = ROOT / "outputs/_smoke/exp3"
    ...
    print(f"exp3  sides={len(jobs)}  n_perm={n_perm}", flush=True)
    t0 = time.time()
