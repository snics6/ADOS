# ADOS_feature — 本番 v4 マルチモーダル特徴量（56 ID）

ADOS 収録の **参加者 ID 単位**（room カメラを時系列結合した 1 セッション）について、本番パイプライン v4 が **正常完了** した **56 ID** の解析 JSON をまとめた配布用フォルダです。
...
以下 **11 ID** は、Pose・Face の `role`（child ↔ examiner）を **全区間反転** した **`multimodal_v3_ce_swap`** の内容を収録しています（2026-07-22 付与リストに基づく）。

| ID |
|----|
| 020404, 020412, 020422, 020443, 020534, 020541 |
| d1_402, d1_405, d1_420, d1_446, d1_450 |
